...
from doc_extractor import extract_all_from_document, extract_from_document
...
class GSTApp(ctk.CTk):
    ...
    def _build_content(self):
        ...
        self._page_bulk(self.pages["bulk"])
    ...
    # ------------------------------------------------------------ Bulk page
    def _page_bulk(self, page):
        self._page_head(page, "Bulk Invoices",
                        "Multi-page PDF (har page 1 invoice) → har row me Weight + "
                        "Amount daalo → sab ek saath generate")

        top = ctk.CTkFrame(page, fg_color="transparent")
        top.pack(fill="x", padx=28, pady=(10, 0))
        ctk.CTkButton(top, text="📁  Choose Multi-Page PDF", width=200, height=38,
                      corner_radius=10, fg_color=INDIGO, hover_color=INDIGO_HOV,
                      font=ctk.CTkFont(size=13, weight="bold"),
                      command=self.on_bulk_pdf).pack(side="left")
        ctk.CTkButton(top, text="🗑  Clear Rows", width=110, height=38,
                      corner_radius=10, fg_color="#eef0fe", hover_color="#e2e5fd",
                      text_color=INDIGO, font=ctk.CTkFont(size=12, weight="bold"),
                      command=self._bulk_clear).pack(side="left", padx=10)
        self.lbl_bulk = ctk.CTkLabel(top, text="koi PDF select nahi hui",
                                     font=ctk.CTkFont(size=12), text_color=MUTED)
        self.lbl_bulk.pack(side="left", padx=6)

        mrow = ctk.CTkFrame(page, fg_color="transparent")
        mrow.pack(fill="x", padx=28, pady=(8, 0))
        ctk.CTkLabel(mrow, text="Amount mode (sab rows):",
                     font=ctk.CTkFont(size=12, weight="bold"),
                     text_color=MUTED).pack(side="left")
        self.bulk_mode = ctk.StringVar(value=MODE_FINAL)
        ctk.CTkSegmentedButton(mrow, values=[MODE_FINAL, MODE_BASE],
                               variable=self.bulk_mode, selected_color=INDIGO,
                               selected_hover_color=INDIGO_HOV,
                               font=ctk.CTkFont(size=12), height=30
                               ).pack(side="left", padx=8)

        self.btn_bulk_gen = ctk.CTkButton(page, text="⚡   Generate ALL Invoices",
                                          height=50, corner_radius=14,
                                          fg_color=INDIGO, hover_color=INDIGO_HOV,
                                          font=ctk.CTkFont(size=16, weight="bold"),
                                          command=self.on_bulk_generate)
        self.btn_bulk_gen.pack(fill="x", padx=28, pady=(12, 0))

        self.bulk_box = ctk.CTkScrollableFrame(page, fg_color=CARD, corner_radius=16)
        self.bulk_box.pack(fill="both", expand=True, padx=28, pady=(12, 20))
        self.bulk_rows = []
        self._bulk_header_done = False

    def _bulk_clear(self):
        for w in self.bulk_box.winfo_children():
            w.destroy()
        self.bulk_rows = []
        self._bulk_header_done = False
        self.lbl_bulk.configure(text="koi PDF select nahi hui", text_color=MUTED)

    def _bulk_header(self):
        cols = [("#", 34), ("Consignee", 180), ("AWB", 118), ("Order ID", 80),
                ("Dest", 56), ("Payment", 92), ("Weight *", 112), ("Amount ₹ *", 100)]
        for c, (title, w) in enumerate(cols):
            ctk.CTkLabel(self.bulk_box, text=title, width=w,
                         font=ctk.CTkFont(size=11, weight="bold"), text_color=MUTED
                         ).grid(row=0, column=c, padx=(4, 4), pady=(6, 4), sticky="w")
        self._bulk_header_done = True

    def _add_bulk_row(self, idx: int, rec: dict):
        if not self._bulk_header_done:
            self._bulk_header()
        r = len(self.bulk_rows) + 1
        pay = rec.get("payment_type") if rec.get("payment_type") in ("Prepaid", "COD") \
            else "Prepaid"
        widgets = {}

        ctk.CTkLabel(self.bulk_box, text=str(idx), width=34,
                     font=ctk.CTkFont(size=12), text_color=MUTED
                     ).grid(row=r, column=0, padx=4, pady=5, sticky="w")
        ctk.CTkLabel(self.bulk_box, text=str(rec.get("consignee_name", "—"))[:24],
                     width=180, font=ctk.CTkFont(size=12), text_color=TEXT,
                     anchor="w").grid(row=r, column=1, padx=4, pady=5, sticky="w")
        ctk.CTkLabel(self.bulk_box, text=str(rec.get("awb", "—")),
                     width=118, font=ctk.CTkFont(size=12), text_color=TEXT,
                     anchor="w").grid(row=r, column=2, padx=4, pady=5, sticky="w")
        ctk.CTkLabel(self.bulk_box, text=str(rec.get("client_order_id", "—"))[:12],
                     width=80, font=ctk.CTkFont(size=12), text_color=TEXT,
                     anchor="w").grid(row=r, column=3, padx=4, pady=5, sticky="w")
        ctk.CTkLabel(self.bulk_box, text=str(rec.get("destination_pincode", "—")),
                     width=56, font=ctk.CTkFont(size=12), text_color=TEXT,
                     anchor="w").grid(row=r, column=4, padx=4, pady=5, sticky="w")
        om = ctk.CTkOptionMenu(self.bulk_box, width=88, height=30, corner_radius=8,
                               fg_color=FIELD, button_color=INDIGO,
                               button_hover_color=INDIGO_HOV, text_color=TEXT,
                               dropdown_fg_color=CARD, values=["Prepaid", "COD"],
                               font=ctk.CTkFont(size=12))
        om.set(pay)
        om.grid(row=r, column=5, padx=4, pady=5)
        e_w = ctk.CTkEntry(self.bulk_box, width=112, height=30, corner_radius=8,
                           fg_color=FIELD, border_color=BORDER, border_width=1,
                           placeholder_text="350 gm", font=ctk.CTkFont(size=12))
        e_w.grid(row=r, column=6, padx=4, pady=5)
        e_a = ctk.CTkEntry(self.bulk_box, width=100, height=30, corner_radius=8,
                           fg_color=FIELD, border_color=BORDER, border_width=1,
                           placeholder_text="82", font=ctk.CTkFont(size=12))
        e_a.grid(row=r, column=7, padx=4, pady=5)
        self.bulk_rows.append({"rec": rec, "e_w": e_w, "e_a": e_a, "pay": om})

    def on_bulk_pdf(self):
        path = filedialog.askopenfilename(
            title="Multi-page Delhivery invoice PDF select karo",
            filetypes=[("PDF files", "*.pdf")])
        if not path:
            return
        try:
            records = extract_all_from_document(path)
        except Exception as exc:  # noqa: BLE001
            messagebox.showerror("Bulk Error", str(exc))
            return
        self._bulk_clear()
        self.lbl_bulk.configure(text=f"{os.path.basename(path)} — {len(records)} "
                                     f"invoice pages mile", text_color=TEXT)
        for i, rec in enumerate(records, 1):
            self._add_bulk_row(i, rec)
        self.log(f"📚 Bulk: {len(records)} shipments load hue — ab har row me "
                 f"Weight + Amount daalo.")

    def on_bulk_generate(self):
        cname = self.e_cname.get().strip()
        if not cname:
            messagebox.showerror("Incomplete", "Pehle Settings me Client Firm Name "
                                              "daaro.")
            return
        rows = []
        incomplete = 0
        for row in self.bulk_rows:
            wt = row["e_w"].get().strip()
            amt_raw = row["e_a"].get().strip().replace(",", "")
            if not wt and not amt_raw:
                continue  # khaali row — skip
            if not wt or not amt_raw:
                incomplete += 1
                continue
            try:
                amt = float(amt_raw)
                ...
                incomplete += 1
                continue
            rows.append((row, wt, amt))
        if not rows:
            messagebox.showwarning("Kuch nahi bana",
                                   "Koi row poora nahi hai — Weight AUR Amount "
                                   "dono chahiye.")
            return
        ...
        self.btn_bulk_gen.configure(state="disabled", text="⏳  Generating…")
        self.update_idletasks()
        ok, fail = 0, []
        for row, wt, amt in rows:
            rec = row["rec"]
            awb = str(rec.get("awb", "")).strip()
            try:
                invoice_num = next_invoice_num(today, counter)
                payload = {
                    "invoice_num": invoice_num, "invoice_date": today,
                    "client_name": cname,
                    "client_address1": self.e_caddr1.get().strip(),
                    "client_address2": self.e_caddr2.get().strip(),
                    "client_gstin": gstin,
                    "client_state": DEFAULT_CLIENT["client_state"],
                    "amount_mode": "inclusive" if self.bulk_mode.get() == MODE_FINAL
                                   else "exclusive",
                    "base_charge": amt,
                    "awb": awb,
                    "consignee_name": str(rec.get("consignee_name", "")),
                    "client_order_id": str(rec.get("client_order_id", "")),
                    "origin_pincode": DEFAULT_CLIENT["origin_pincode"],
                    "destination_pincode": str(rec.get("destination_pincode", "")),
                    "weight": wt,
                    "payment_method": row["pay"].get(),
                }
                safe = self._safe(payload["consignee_name"])
                out_pdf = os.path.join(BASE_DIR, "generated",
                                       f"GST_invoice_{safe}_{awb}.pdf")
                result = generate_gst_invoice(payload, out_pdf)
                archived = archive_invoice(out_pdf, None, today, cname,
                                           archive_root=BASE_DIR)
                ok += 1
                self.log(f"✅ [{ok}/{len(rows)}] {result['invoice_num']} — "
                         f"{payload['consignee_name']} — ₹{result['total']}")
                self.log(f"      📁 {archived}")
            except Exception as exc:  # noqa: BLE001
                fail.append(f"{rec.get('consignee_name', '?')}: {exc}")
                self.log(f"❌ Fail: {rec.get('consignee_name', '?')} — {exc}")
        self.btn_bulk_gen.configure(state="normal",
                                    text="⚡   Generate ALL Invoices")
        msg = (f"HO GAYA! 🎉\n\n{ok}/{len(rows)} invoices ban gayi "
               f"(archive me saved).")
        if incomplete:
            msg += f"\n\n⚠️ {incomplete} row aadhi thi (skip) — Weight AUR Amount dono daalo."
        if fail:
            msg += f"\n\n❌ {len(fail)} fail:"
            msg += "\n" + "\n".join(fail[:5])
        messagebox.showinfo("Bulk Complete", msg)
        self.log(f"📚 Bulk complete: {ok} OK, {len(fail)} fail, {incomplete} skip.")
