...
def normalize_gst_data(data: dict) -> dict:
    ...
    return {
        ...
        "sac": SAC_CODE,
        "payment_method": str(d.get("payment_method") or d.get("payment_type")
                              or "").strip(),
...
def build_gst_story(d: dict) -> list:
    ...
    specs = Table(
        [[
            ...
            Paragraph("Payment Method", s_sec),
        ], [
            ...
            Paragraph(str(d["payment_method"] or "—"), s_body_b),
        ]],
        colWidths=[118, 118, 132, 142],
    )
...
<div class="card">
  ...
  <div class="box">
    ...
    <div class="specs">
      ...
      <div><p class="sh">Weight</p><p class="sm b" style="font-size:10pt;">{e(str(d['weight'] or '—'))}</p></div>
      <div><p class="sh">Payment Method</p><p class="sm b" style="font-size:10pt;">{e(str(d['payment_method'] or '—'))}</p></div>
    </div>
  </div>
  <table class="fin">
...
def archive_invoice(output_pdf: str, output_html: str = None, invoice_date: str = "",
    ...
    folder = folder.title()      # "SHREE MAHADEV" → "Shree Mahadev" (consistent archive)
    base = archive_root or PROJECT_ROOT          # archive_root = base location
    root = os.path.join(base, folder)            # client-name folder HAMESHA
