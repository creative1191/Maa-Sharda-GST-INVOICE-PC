...
def extract_all_from_document(path: str) -> list:
    """
    MULTI-PAGE Delhivery PDF → LIST of shipment dicts (har page = 1 invoice).
    Bulk invoice generation ke liye.
    """
    ext = os.path.splitext(path)[1].lower().lstrip(".")
    if ext != "pdf":
        raise ValueError("Bulk mode sirf multi-page PDF ke liye hai "
                         "(har page par 1 Delhivery invoice)")
    from pdf_parser import parse_delhivery_pdf
    ...
    for r in records:
        ...
        out.append({
            ...
            "client_order_id": r.get("sales_number", ""),
            "payment_type": r.get("payment_type", ""),
            "_source": f"page-{r.get('page', '?')}",
        })
    if not out:
        raise ValueError("Is PDF me koi Delhivery invoice page nahi mili")
    return out
